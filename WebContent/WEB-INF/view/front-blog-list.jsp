      ...
      <div class="row">
        ...
        <main class="posts-listing col-lg-8"> 
          <div class="container">
		    ...
		    <c:set value="${blogList}" var="blogPageList" />
			
			<c:forEach items="${blogPageList.pageList}" var="blog">
	                ...
	                <div class="post-details">
	                  ...
	                  <footer class="post-footer d-flex align-items-center"><a href="/BlogProject/front-blog-user/${blog.user.seoName}/${blog.user.username}" class="author d-flex align-items-center flex-wrap">
	                    ...
	                    <div class="comments meta-last"><i class="icon-comment"></i>${blog.commentsNum}</div>
	                  </footer>
	                </div>
	              </div>
            ...
            <nav aria-label="Page navigation example">
			  ...
			  <%-- Show Prev as link if not on first page --%>
					  <c:when test="${blogPageList.firstPage}">
					    <span>Prev</span>
					  </c:when>
					  <c:otherwise>
					    <c:url value="/front-blog-list/prev" var="url" />                  
               ...
               <br><br> <br><br>	
                ...
                <li class="page-item">
                <c:forEach begin="1" end="${blogPageList.pageCount}" step="1"  varStatus="tagStatus">
			  <c:choose>
			    <%-- In PagedListHolder page count starts from 0 --%>
			    <c:when test="${(blogPageList.page + 1) == tagStatus.index}">
			      <span>${tagStatus.index}</span>
			    </c:when>
			    <c:otherwise>
			      <c:url value="/front-blog-list/${tagStatus.index}" var="url" />                  
			  ...
			  <c:when test="${blogPageList.lastPage}">
			    <span>Next</span>
			  </c:when>
			  <c:otherwise>
			    <c:url value="/front-blog-list/next" var="url" />                  
